...
CREATE POLICY users_select ON users FOR SELECT USING (
  ...
  OR (auth.user_role() = 'manager' AND manager_id = auth.user_id())
);
...
CREATE POLICY enrollments_select ON enrollments FOR SELECT USING (
  ...
  OR auth.user_role() IN ('admin', 'super_admin')
  OR (auth.user_role() = 'manager' AND user_id IN (
    SELECT id FROM users WHERE manager_id = auth.user_id()
  ))
);

CREATE POLICY enrollments_insert ON enrollments FOR INSERT WITH CHECK (
  user_id = auth.user_id()
  OR auth.user_role() IN ('admin', 'super_admin', 'manager')
);
