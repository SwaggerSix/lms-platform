...
export default function LearnerDashboardClient({ data }: { data: LearnerDashboardData }) {
  ...
  const widgets: Record<string, React.ReactNode> = {
    welcome_banner: !w.welcome_banner ? null : (
      <div key="welcome_banner" className="relative overflow-hidden rounded-2xl bg-gradient-to-r from-indigo-600 to-purple-600 px-8 py-8 text-white shadow-lg">
        <div className="relative z-10">
          <h1 className="text-2xl font-bold">Welcome back, {data.userName}!</h1>
          <p className="mt-1 text-indigo-100">You&apos;re on a 7-day learning streak. Keep it going!</p>
    ...
    spotlight: !w.spotlight || data.spotlightCourses.length === 0 ? null : (
      <section key="spotlight">
        ...
        <div className="grid grid-cols-1 gap-4 md:grid-cols-2 lg:grid-cols-3">
          {data.spotlightCourses.map((course) => (
            <div key={course.id} className="group relative overflow-hidden rounded-xl border border-gray-200 bg-white shadow-sm transition-all hover:-translate-y-0.5 hover:shadow-lg">
              <div className={cn("relative flex h-40 items-end p-4", course.thumbnail)}>
                <div className="absolute inset-0 bg-gradient-to-t from-black/60 to-transparent" />
                <span className={cn("absolute right-3 top-3 rounded-full px-2.5 py-0.5 text-[11px] font-bold uppercase tracking-wide", course.badge === "Most Popular" ? "bg-amber-400 text-amber-900" : course.badge === "New" ? "bg-emerald-400 text-emerald-900" : "bg-red-400 text-white")}>
                  {course.badge}
                </span>
                <div className="relative z-10">
                  <span className="rounded-full bg-white/20 px-2 py-0.5 text-[11px] font-medium text-white backdrop-blur-sm">{course.type}</span>
    ...
    continue_learning: !w.continue_learning || data.inProgressCourses.length === 0 ? null : (
      <section key="continue_learning">
        <div className="mb-4 flex items-center justify-between">
            ...
            <div key={course.id} className="group overflow-hidden rounded-xl border border-gray-200 bg-white shadow-sm transition-shadow hover:shadow-md">
              ...
              <div className="p-4">
                <div className="mb-1 flex items-center gap-2">
                  <span className="rounded-full bg-indigo-50 px-2 py-0.5 text-[11px] font-medium text-indigo-600">{course.category}</span>
                </div>
                ...
                <div className="mt-3">
                  ...
                  <div className="h-2 overflow-hidden rounded-full bg-gray-100">
                    <div className="h-full rounded-full bg-indigo-600 transition-all" style={{ width: `${course.progress}%` }} />
                  </div>
                </div>
              </div>
            </div>
            ...
            <tbody className="divide-y divide-gray-50">
              {data.upcomingDeadlines.length > 0 ? data.upcomingDeadlines.map((deadline) => (
                <tr key={deadline.id} className="transition-colors hover:bg-gray-50">
                  <td className="px-4 py-3"><p className="text-sm font-medium text-gray-900">{deadline.course}</p></td>
                  <td className="hidden px-4 py-3 sm:table-cell">
                    <span className={cn("inline-flex rounded-full px-2 py-0.5 text-xs font-medium", deadline.type === "Mandatory" ? "bg-red-50 text-red-700" : deadline.type === "Compliance" ? "bg-amber-50 text-amber-700" : "bg-gray-100 text-gray-600")}>
                      {deadline.type}
                    </span>
                  </td>
                  ...
                  <td className="px-4 py-3">
                    <span className={cn("inline-flex items-center rounded-full px-2 py-0.5 text-xs font-medium", deadline.status === "urgent" ? "bg-red-50 text-red-700" : deadline.status === "upcoming" ? "bg-amber-50 text-amber-700" : "bg-green-50 text-green-700")}>
                ...
                <tr>
                  <td colSpan={4} className="px-4 py-8 text-center text-sm text-gray-500">No upcoming deadlines</td>
                </tr>
              )}
            </tbody>
          </table>
      ...
      <section key="achievements">
        ...
        <div className="space-y-3">
          {recentAchievements.map((achievement) => {
            ...
            return (
              <div key={achievement.id} className="flex items-center gap-3 rounded-xl border border-gray-200 bg-white p-4 shadow-sm transition-shadow hover:shadow-md">
  ...
  return (
    <div className="mx-auto max-w-7xl space-y-8">
      ...
      {data.widgetOrder.map((key) => widgets[key] ?? null)}
    </div>
  );
}
