...
<tr><td style="padding:20px 32px;border-top:1px solid #e5e7eb;background:#f9fafb;"><p style="margin:0;color:#6b7280;font-size:12px;text-align:center;">${storeName} — thank you for your business.</p></td></tr>
...
function lineItemsTable(data: OrderEmailData): string {
  const rows = data.lines
    .map(
      (l) => `<tr>
<td style="padding:10px 0;border-bottom:1px solid #eef2f7;color:#111827;font-size:14px;">${l.name}<div style="color:#6b7280;font-size:12px;">${l.quantity} ${l.quantity === 1 ? "seat" : "seats"} × ${money(l.unitPrice, data.currency)}</div></td>
...
Hi ${data.customerName}, thank you${data.companyName ? ` — we've received your order for <strong>${data.companyName}</strong>` : ""}. Your order number is <strong>${data.orderNumber}</strong>.
</p>
${lineItemsTable(data)}
...
<tr><td style="padding:3px 0;color:#6b7280;font-size:13px;">Company</td><td style="padding:3px 0;text-align:right;color:#111827;font-size:13px;">${data.companyName || "—"}</td></tr>
...
${data.manageUrl ? `<p style="margin:16px 0 0;font-size:14px;"><a href="${data.manageUrl}" style="color:${data.brandColor};font-weight:600;">Open in admin →</a></p>` : ""}
`;
  return {
    subject: `New order ${data.orderNumber} — ${data.companyName || data.customerName} (${money(data.total, data.currency)})`,
    html: layout(data.brandColor, data.storeName, inner),
    text: `New order ${data.orderNumber} from ${data.companyName || data.customerName} (${data.customerEmail}). Total ${money(data.total, data.currency)}.`,
  };
}
...
<p style="margin:0;color:#6b7280;font-size:13px;">Need a quote, a PO, or have questions about scheduling? Just reply — we're happy to help.</p>
`;
  return {
